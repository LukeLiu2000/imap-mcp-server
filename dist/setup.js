...
var ImapService = class {
   ...
   */
  async searchEmails(accountId, folderName, criteria, options2) {
    const client = await this.ensureConnected(accountId);
    const includeBody = options2?.includeBody === true;
    const bodyMaxLength = options2?.bodyMaxLength ?? DEFAULT_BODY_MAX_LENGTH;
    let lock;
    try {
      ...
      for await (const msg of client.fetch(uids, fetchQuery, { uid: true })) {
        ...
        try {
          const rendered = await this.buildEmailContentFromSource(msg.uid, msg.source, msg.flags, {
            bodyFormat: options2?.bodyFormat ?? DEFAULT_BODY_FORMAT,
            bodyMaxLength,
            includeAttachmentText: false
          });
          messages.push(this.mergeBodyIntoMessage(base, rendered, options2?.bodyFormat ?? DEFAULT_BODY_FORMAT));
        } catch {
          messages.push(base);
        }
      }
  ...
  async getLatestEmails(accountId, folderName, count, options2) {
    const client = await this.ensureConnected(accountId);
    const includeBody = options2?.includeBody === true;
    const bodyMaxLength = options2?.bodyMaxLength ?? DEFAULT_BODY_MAX_LENGTH;
    let lock;
  ...
  async getEmailContent(accountId, folderName, uid, options2 = {}) {
    const client = await this.ensureConnected(accountId);
    let lock;
    try {
      lock = await client.getMailboxLock(folderName);
      ...
      return await this.buildEmailContentFromSource(uid, source.source, source.flags, options2);
    } finally {
      if (lock) {
        lock.release();
      }
   ...
   */
  async buildEmailContentFromSource(uid, source, flags, options2 = {}) {
    const {
      includeAttachmentText = false,
   ...
   */
  async moveEmail(accountId, folderName, uids, targetFolder, options2) {
    const isBatch = Array.isArray(uids);
    const uidList = isBatch ? uids : [uids];
    const client = await this.ensureConnected(accountId);
    let destinationCreated = false;
    if (options2?.createDestinationIfMissing) {
  ...
  async createFolder(accountId, folderPath) {
    ...
    try {
      ...
      const path4 = result && typeof result === "object" && "path" in result ? result.path : folderPath;
   ...
   */
  async findThreadMessages(accountId, sourceFolder, searchFolder, options2) {
    const client = await this.ensureConnected(accountId);
    const includeReferences = options2?.searchReferences !== false;
    const includeBody = options2?.includeBody === true;
    const bodyMaxLength = options2?.bodyMaxLength ?? DEFAULT_BODY_MAX_LENGTH;
    const bodyFormat = options2?.bodyFormat ?? DEFAULT_BODY_FORMAT;
    const messageIds = [];
...
import os2 from "os";
program.name("imap-setup").description("IMAP MCP Server Setup Wizard").option("-p, --port <port>", "Port for web UI", "3000").option("--no-open", "Do not open browser automatically").option("--claude-setup", "Setup Claude Desktop integration").option("--skip-claude", "Skip Claude Desktop integration").parse();
var options = program.opts();
async function getClaudeConfigPath() {
  const platform = os2.platform();
  switch (platform) {
    case "darwin":
      return path3.join(os2.homedir(), "Library", "Application Support", "Claude", "claude_desktop_config.json");
    case "win32":
      return path3.join(os2.homedir(), "AppData", "Roaming", "Claude", "claude_desktop_config.json");
    case "linux":
...
async function setupClaudeIntegration() {
  ...
  try {
    ...
    const serverPath = path3.join(process.cwd(), "dist", "index.js");
    let config = {};
    try {
      const existingConfig = await fs3.readFile(configPath, "utf-8");
    ...
    console.log("\n" + chalk.yellow("\u{1F4A1}") + " You can manually add this configuration to your Claude Desktop config:");
    console.log(chalk.gray("  {"));
    console.log(chalk.gray('    "mcpServers": {'));
    console.log(chalk.gray('      "imap": {'));
    console.log(chalk.gray('        "command": "node",'));
    console.log(chalk.gray(`        "args": ["${path3.join(process.cwd(), "dist", "index.js")}"]`));
    console.log(chalk.gray("      }"));
    console.log(chalk.gray("    }"));
    console.log(chalk.gray("  }"));
  }
}
async function askForClaudeSetup() {
...
async function main() {
  console.log(chalk.blue.bold("\n\u{1F680} IMAP MCP Server Setup Wizard\n"));
  ...
  try {
    ...
    console.log("\n" + chalk.blue("\u{1F4E7}") + " After configuring your email accounts:");
